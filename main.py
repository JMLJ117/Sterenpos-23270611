...
from models.ventas import VentasScreen, HistorialVentasScreen
...
class ProductosScreen(Screen):
    ...
    def agregar_producto(self):
        solicitar_datos(['Nombre','Precio','ProveedorID'], lambda d: (agregar_producto(d), ver_productos_gui(self.ids.rv)))
    def actualizar_producto(self):
        solicitar_datos(['ProductoID','Nombre','Precio','ProveedorID'], lambda d: (actualizar_producto(d), ver_productos_gui(self.ids.rv)))
    def eliminar_producto(self):
        solicitar_datos(['ProductoID'], lambda d: (eliminar_producto(d['ProductoID']), ver_productos_gui(self.ids.rv)))

class UsuariosScreen(Screen):
    def on_enter(self): ver_usuarios_gui(self.ids.rv)
    def agregar_usuario(self):
        solicitar_datos(['Nombre','Correo','RolID'], lambda d:(agregar_usuario(d), ver_usuarios_gui(self.ids.rv)))
    def actualizar_usuario(self):
        solicitar_datos(['UsuarioID','Nombre','Correo','RolID'], lambda d:(actualizar_usuario(d), ver_usuarios_gui(self.ids.rv)))
    def eliminar_usuario(self):
        solicitar_datos(['UsuarioID'], lambda d:(eliminar_usuario(d['UsuarioID']), ver_usuarios_gui(self.ids.rv)))

class ClientesScreen(Screen):
    def on_enter(self): ver_clientes_gui(self.ids.rv)
    def agregar_cliente(self):
        solicitar_datos(['Nombre','Telefono','Direccion'], lambda d:(agregar_cliente(d), ver_clientes_gui(self.ids.rv)))
    def actualizar_cliente(self):
        solicitar_datos(['ClienteID','Nombre','Telefono','Direccion'], lambda d:(actualizar_cliente(d), ver_clientes_gui(self.ids.rv)))
    def eliminar_cliente(self):
        solicitar_datos(['ClienteID'], lambda d:(eliminar_cliente(d['ClienteID']), ver_clientes_gui(self.ids.rv)))

class ProveedoresScreen(Screen):
    def on_enter(self): ver_proveedores_gui(self.ids.rv)
    def agregar_proveedor(self):
        solicitar_datos(['Nombre','Telefono','Direccion'], lambda d:(agregar_proveedor(d), ver_proveedores_gui(self.ids.rv)))
    def actualizar_proveedor(self):
        solicitar_datos(['ProveedorID','Nombre','Telefono','Direccion'], lambda d:(actualizar_proveedor(d), ver_proveedores_gui(self.ids.rv)))
    def eliminar_proveedor(self):
        solicitar_datos(['ProveedorID'], lambda d:(eliminar_proveedor(d['ProveedorID']), ver_proveedores_gui(self.ids.rv)))
...
class SterenApp(App):
    def build(self):
        ...
        sm.add_widget(HistorialVentasScreen(name="historial_ventas"))
        sm.current = 'login'
        return sm
